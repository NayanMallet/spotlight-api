...
export class SportsScraperService {
  ...
  private knownVenues: Record<string, { lat: number; lng: number }> = {
    ...
    'Stadium de Toulouse': { lat: 43.5833, lng: 1.434 },
  ...
  constructor(private eventsService: EventsService) {
    ...
    if (!this.rapidApiKey) {
      console.warn(
        '[SportsScraper] No API Key found. Please set RAPID_API_KEY or X-RapidAPI-Key in .env'
      )
    }
  }
  ...
  async fetchAndSaveSportsEvents() {
    ...
    for (const [sportName, sportId] of Object.entries(this.sportsMap)) {
      ...
      for (let i = 0; i < daysToCheck; i++) {
        ...
        try {
          const response = await this.fetchFromApi<{ data: SportEvent[] }>(
            `/sports/${sportId}/events/date/${dateStr}`
          )
          ...
          console.warn(
            `[SportsScraper] Failed to fetch events for ${sportName} on ${dateStr}:`,
            err
          )
        }
      }
  ...
  private async processEvent(event: SportEvent, sportName: string, date: DateTime) {
    try {
      ...
      if (placeName === 'Stadium / Arena' || !event.venue_id) {
        if (
          sportName === 'football' &&
          (homeName.includes('Toulouse') || homeName.includes('TFC'))
        ) {
          ...
          homeName.includes('Toulouse Métropole') ||
          homeName.includes('Stade Toulousain')
        ) {
