...
test.group('Auth - Get User Controller', () => {
  ...
  test('should return 401 for unauthenticated request', async ({ client }: { client: ApiClient }) => {
    const response = await client
      .get('/users/me')
  ...
  test('should return 401 for expired token', async ({ client }: { client: ApiClient }) => {
    ...
    const response = await client
      .get('/users/me')
      .bearerToken('oat_1.expired_token_here')
  ...
  test('should handle concurrent requests for same user', async ({ client }: { client: ApiClient }) => {
    // Register a user
    const registerResponse = await client
      .post('/register')
      .json({
    ...
    const promises = Array(5).fill(null).map(() =>
      client.get('/users/me').bearerToken(token)
    )
