...
test.group('Auth - Register Controller', () => {
  ...
  test('should register user without optional bannerUrl', async ({ client }: { client: ApiClient }) => {
    const response = await client
      .post('/register')
      .json({
        full_name: 'Jane Doe',
  ...
  test('should return 400 if user is already logged in', async ({ client }: { client: ApiClient }) => {
    ...
    const secondRegisterResponse = await client
      .post('/register')
      .bearerToken(token)
      .json({
  ...
  test('should return 422 for missing required fields', async ({ client }: { client: ApiClient }) => {
    const response = await client
      .post('/register')
      .json({
        email: 'test@example.com'
  ...
  test('should return 422 for invalid bannerUrl format', async ({ client }: { client: ApiClient }) => {
    const response = await client
      .post('/register')
      .json({
        full_name: 'Test User',
