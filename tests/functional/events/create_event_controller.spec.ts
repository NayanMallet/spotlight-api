...
test.group('Events - Create Event Controller', () => {
  ...
  test('should return 401 for unauthenticated event creation', async ({ client }: { client: ApiClient }) => {
    const eventData = {
  ...
  test('should return 422 for end date before start date', async ({ client }: { client: ApiClient }) => {
    // Register and login a user
    const registerResponse = await client
      .post('/register')
      .json({
  ...
  test('should create event with minimal required fields', async ({ client }: { client: ApiClient }) => {
    // Register and login a user
    const registerResponse = await client
      .post('/register')
      .json({
